...
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetDailyReport_DBA_20210226]
...
BEGIN
	...
	INNER	JOIN dbo.META2_M_LastCollectInfo d WITH (NOLOCK) 
	ON		a.InstanceID = d.InstanceID 
	AND		d.CollectItem = N'WhoIsActive'
	CROSS	APPLY (
	...
	/* 쿼리 몸체 */
	SELECT   sa.CollectDate
		    ,ii.InstanceID
		    ,ii.ServerName                                          AS "업무"
		    ,sb.ServiceIP                                           AS "IP"
		    ,sa.ClusterOwnerNode                                    AS "클러스터 Active 노드"
		    ,CASE WHEN sb.IsClustered = 1 THEN 'Y'
			 	  ELSE 'N' 
             END                                                    AS "클러스터 구성 여부"
			...
			,tb.BlockingWaittime									AS "Blocking(ms)"
	...
	LEFT    OUTER JOIN [dbo].[TBL2_M_ServerBasicInfo] AS sb         -- 서버 기본 정보 (매일 0:00시 수집)
	ON      ii.InstanceID = sb.InstanceID 
    AND     CONVERT(varchar(8), sb.CollectDate, 112) = CONVERT(varchar(8), GETDATE(), 112)
	LEFT	OUTER JOIN @TABLEBlocking tb																							-- [Blocking 정보 JOIN]
	ON		ii.InstanceID = tb.InstanceID
    LEFT    OUTER JOIN (																										    -- [클러스터 정보 JOIN]
						SELECT   ClusterOwnerNode
						...
						GROUP   BY ClusterOwnerNode
						) AS sa
    ON      ii.InstanceID = sa.InstanceID
	...
	ON      ii.InstanceID = aj.InstanceID
    LEFT    OUTER JOIN (                                                                                                            -- [Error 정보 JOIN]
                        SELECT   instanceID
						        ,COUNT(*) AS "ErrorCount"
    ...
    ON      ii.InstanceID = er.InstanceID
	LEFT    OUTER JOIN (																							-- 물리 메모리 사용량 JOIN -> 수정 필요
					    ...
					    INNER   JOIN (
							          ...
							          ON AA.InstanceID = BB.InstanceID AND AA.CollectDate = BB.CollectDate
							        WHERE CounterName = 'Memory\Available MBytes'
							        ) AS B
					    ON      A.InstanceID = B.InstanceID 
                        AND     CONVERT(varchar(8), A.CollectDate, 112) = CONVERT(varchar(8), B.CollectDate, 112)
				GROUP   BY A.InstanceID
				    ) AS um
	ON      ii.InstanceID = um.InstanceID
	LEFT OUTER JOIN (																						-- 디스크 사용량 JOIN		
				SELECT InstanceID,
				...
				FROM (SELECT INSTANCEID, DRIVE, USEDDISK, 
				...
				FROM (
                        ...
                        INNER   JOIN(
                                     SELECT   CONVERT(VARCHAR(10), CollectDate, 23) AS "CollectDate"
                                             ,InstanceID
                                             ,Drive
                                             ,CONVERT(decimal(5, 2), ROUND(MAX(TotalSpaceMB - FreeSpaceMB) / MAX(TotalSpaceMB) * 100, 2)) AS "UsedDisk"
                                     FROM    TBL2_M_DatabaseDiskUsage                                                                   -- 디스크 사용량 정보 (매일 7:30시 수집)
                                     WHERE   CollectDate >= CONVERT(VARCHAR(10), DATEADD(DAY, -1, GETDATE()), 23) + ' 00:00:00.000'
                                     AND     CollectDate < CONVERT(VARCHAR(10), DATEADD(DAY, 1, GETDATE()), 23) + ' 00:00:00.000'
                                     GROUP   BY CONVERT(VARCHAR(10), CollectDate, 23), InstanceID, Drive
                                    ) AS iB
                        ON      iA.CollectDate = iB.CollectDate
                        AND     iA.InstanceID = iB.InstanceID)AS X
					) AS A
	...
	ON ii.InstanceID = dk.InstanceID
	WHERE ii.UseYN = 'Y'
	/* 쿼리 몸체 */
END
