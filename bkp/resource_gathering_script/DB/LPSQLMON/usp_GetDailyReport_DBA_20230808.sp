...
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetDailyReport_DBA_20230808]
...
BEGIN
	...
	LEFT OUTER JOIN [dbo].[TBL2_M_ServerBasicInfo] AS sb ON ii.InstanceID = sb.InstanceID AND CONVERT(varchar(8), sb.CollectDate, 112) = CONVERT(varchar(8), GETDATE(), 112) -- 서버 기본 정보 (매일 0:00시 수집)
	...
	) AS sa ON ii.InstanceID = sa.InstanceID
    ...
    OUTER APPLY (                                                                                                            -- [Error 정보 JOIN]
        SELECT   instanceID
				,COUNT(DISTINCT CONVERT(VARCHAR,LOG_DATE) + PROCESS_INFO + ERRORLOG_TEXT) AS "ErrorCount"
	...
	LEFT OUTER JOIN (																						-- 디스크 사용량 JOIN		
		...
		GROUP BY InstanceID
	) AS dk ON ii.InstanceID = dk.InstanceID
	WHERE ii.UseYN = 'Y'
END
