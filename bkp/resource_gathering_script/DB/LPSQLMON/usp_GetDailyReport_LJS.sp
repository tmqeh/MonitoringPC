...
-- Execute : EXEC usp_GetDailyReport_LJS '20250721'
-- =============================================
...
BEGIN
	...
	---------------------------------------------------------------------------
	DECLARE @var_Date DATETIME = IIF(ISDATE(@in_DATE)=1, CONVERT(DATE, @in_DATE), GETDATE()) 
	DECLARE @DateSearchTo DATETIME = @var_Date
	...
	FROM (
		...
		WHERE CollectDate >= CONVERT(VARCHAR(10), @var_Date, 23) + ' 00:00:00.000'
		AND CollectDate < CONVERT(VARCHAR(10), DATEADD(DAY, 1, @var_Date), 23) + ' 00:00:00.000'
	) sa
	WHERE sa.RN = 1
	...
	FROM (
		...
		FROM (
           ...
           INNER   JOIN(
				...
				WHERE   CollectDate >= CONVERT(VARCHAR(10), DATEADD(DAY, -1, @var_Date), 23) + ' 00:00:00.000'
				AND     CollectDate < CONVERT(VARCHAR(10), DATEADD(DAY, 1, @var_Date), 23) + ' 00:00:00.000'
	...
	INNER LOOP JOIN [dbo].[TBL2_M_ServerBasicInfo] AS sb ON ii.InstanceID = sb.InstanceID AND sb.CollectDate BETWEEN CONVERT(varchar(10), @var_Date, 121) AND CONVERT(varchar(10), @var_Date, 121) + ' 23:59:59.997' -- 서버 기본 정보 (매일 0:00시 수집)
    ...
    LEFT OUTER JOIN (                                                                                                            -- [AgentJob 정보 JOIN]
		...
		WHERE LastRunDateTime >= CONVERT(varchar(10), DATEADD(DAY, -1, @var_Date), 23) + ' 00:00:00.000' --전일부터 당일까지의 실패 수집
	...
	LEFT OUTER JOIN (
		...
		FROM (
			...
			WHERE   LOG_DATE >= CONVERT(varchar(10), DATEADD(DAY, -1, @var_Date), 23) + ' 00:00:00.000'         --전일부터 당일까지의 에러 로그 수집
          ...
          FROM (
                SELECT InstanceID
                 ...
                 WHERE 1=1
        		   ...
        		   AND CollectDate >= SUBSTRING(CONVERT(VARCHAR, @var_Date, 120), 1, 11) + ' 00:00:00'
               ) CPU
         WHERE CPU.RN = 1
