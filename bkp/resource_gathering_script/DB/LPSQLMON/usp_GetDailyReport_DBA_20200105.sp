...
-- =============================================
CREATE PROCEDURE [dbo].[usp_GetDailyReport_DBA_20200105]
...
BEGIN
	...
	/* 쿼리 몸체 */
	INSERT INTO [dbo].[TB_DailyReport_S] ([CLCT_DTM],
										  [INST_ID], 
										  [JOB_NM],
										  [IP_ADDR], 
										  [CLU_NODE_NM],
	...
	LEFT    OUTER JOIN [dbo].[TBL2_M_ServerBasicInfo] AS sb         -- 서버 기본 정보 (매일 0시 수집)
	...
	LEFT OUTER JOIN (																						-- 디스크 사용량 JOIN		
				...
				FROM (SELECT INSTANCEID, DRIVE, USEDDISK, 
								...
								END  as USED_RNK
				FROM (SELECT InstanceID, Drive,
							CONVERT(decimal(5, 2), ROUND(MAX(TotalSpaceMB - FreeSpaceMB) / MAX(TotalSpaceMB) * 100, 2)) AS "UsedDisk",
							RANK() OVER(PARTITION BY InstanceID ORDER BY drive) LETTER_RNK,
							RANK() OVER(PARTITION BY InstanceID ORDER BY drive) USED_RNK
					FROM TBL2_M_DatabaseDiskUsage 
					WHERE CONVERT(varchar(8), CollectDate, 112) = CONVERT(varchar(8), GETDATE(), 112)
					GROUP BY InstanceID, Drive) AS X
					) AS A
